	...
	<view class="content">
		...
		<view class="language-pick">
			<view class="pick">
				...
				<span class="reverseLang iconfont icon-zhuanhuan" @touchstart="reverseLang"></span>
				<picker
					class="targetLang"
					mode="selector"
					:range="langkeys"
...
export default {
	data() {
		return {
			...
			result: '',
			langkeys: [
				{ index: 0, type: 'auto', value: '自动检测' },
				{ index: 1, type: 'zh', value: '中文' },
				{ index: 2, type: 'en', value: '英语' },
				{ index: 3, type: 'wyw', value: '文言文' },
				{ index: 4, type: 'jp', value: '日语' },
				{ index: 5, type: 'kor', value: '韩语' },
				{ index: 6, type: 'fra', value: '法语' },
				{ index: 7, type: 'ru', value: '俄语' },
				{ index: 8, type: 'it', value: '意大利语' },
				{ index: 9, type: 'cht', value: '繁体中文' }
			],
	...
	methods: {
		...
		translate() {
			...
			request({
				url: editUrl({
					from: this.originLang.type,
					to: this.targetLang.type,
					query: this.query.trim()
				})
			}).then(res => {
				if (res.statusCode === 200 && !res.data.error_code) {
					const dts = res.data.trans_result;
					const str = dts.reduce((pre, cur) => `${pre}${cur.dst}\n`, '');
					this.result = str;
					uni.hideLoading();
					// 加入翻译历史
					const his = storage.get(config.hiskey);
					his.unshift({
						origin: this.query.trim(),
						target: str,
						id: Date.now()
					});
					storage.set(config.hiskey, his);
				} else {
					uni.showToast({
